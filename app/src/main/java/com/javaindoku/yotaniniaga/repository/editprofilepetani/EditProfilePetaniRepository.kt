...
class EditProfilePetaniRepository @Inject constructor(private val api: Api) {

    fun editProfilePetani(userToken: String, body: RequestBody) : MutableLiveData<ApiResult<EditProfile>> {
        val result = MutableLiveData<ApiResult<EditProfile>>()
        result.postValue(ApiResult.loading())
        GlobalScope.launch {
            try {
                val response = api.editProfilePetani(body, userToken.authorization())
